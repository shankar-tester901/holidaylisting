			...
			if (typeof obj[element] === 'undefined') {
				if (throwErr) {
					throw new ValidationError({
						...
						message: `Value for property ${element} cannot be null or empty in ${objName}`
					});
				}
				return false;
			}
...
function isNonNullNumber(value, name, throwErr) {
	...
	if (throwErr) {
		throw new ValidationError({
			code: 'invalid-argument',
			message: `Value provided for ${name} must be a non empty or non null number.`
		});
	}
	return false;
}
...
function isNonEmptyString(value, name, throwErr) {
	...
	if (throwErr) {
		throw new ValidationError({
			code: 'invalid-argument',
			message: `Value provided for ${name} must be a non empty or non null string.`
		});
	}
	return false;
}
...
function isNonNullObject(value, name, throwErr) {
	...
	if (throwErr) {
		throw new ValidationError({
			code: 'invalid-argument',
			message: `Value provided for ${name} must be a non empty or non null object.`
		});
	}
	return false;
}
...
function isNonNullArray(value, name, throwErr) {
	...
	if (throwErr) {
		throw new ValidationError({
			code: 'invalid-argument',
			message: `Value provided for ${name} must be a non empty or non null Array.`
		});
	}
	return false;
}
...
function isStringOrNumber(value, name, throwErr) {
	...
	if (throwErr) {
		throw new ValidationError({
			code: 'invalid-argument',
			message: `Value provided for ${name} must be a non empty String or Number.`
		});
	}
	return false;
}
